...
export const getDeliveryArea = async (req, reply) => {
  try {
    ...
    if (role !== 'Seller') {
      return reply.status(403).send({ success: false, message: 'Access denied. Seller role required.' });
    }
    ...
    return reply.status(500).send({ success: false, message: 'Failed to fetch delivery area' });
  }
};
...
export const setDeliveryArea = async (req, reply) => {
  try {
    ...
    if (radius === undefined || radius === null) {
      return reply.status(400).send({ success: false, message: 'Radius is required' });
    }
    if (typeof radius !== 'number' || radius < 0 || radius > 20) {
      return reply.status(400).send({ success: false, message: 'Radius must be a number between 0 and 20' });
    }
    if (unit && !['km', 'miles'].includes(unit)) {
      return reply.status(400).send({ success: false, message: 'Unit must be either "km" or "miles"' });
    }
    const seller = await Seller.findById(userId);
    if (!seller) {
      return reply.status(404).send({ success: false, message: 'Seller not found' });
    ...
    if (!seller.storeLocation || !seller.storeLocation.isSet) {
      return reply.status(400).send({ success: false, message: 'Please set your store location before setting delivery area' });
    }
    seller.deliveryArea = {
    ...
    return reply.send({ success: true, message: 'Delivery area updated successfully', data: {
    ...
    return reply.status(500).send({ success: false, message: 'Failed to update delivery area' });
  }
};
...
export const clearDeliveryArea = async (req, reply) => {
  try {
    ...
    seller.deliveryArea = { radius: 0, unit: 'km', isActive: false, updatedAt: new Date() };
    await seller.save();
    console.log('Delivery area cleared for seller ' + userId);
    return reply.send({ success: true, message: 'Delivery area cleared successfully' });
  } catch (error) {
    console.error('Clear Delivery Area Error:', error);
    return reply.status(500).send({ success: false, message: 'Failed to clear delivery area' });
  }
};
